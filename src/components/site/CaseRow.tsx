...
export function CaseRow({ item, level = 3 }: CaseRowProps) {
  ...
  return (
    ...
    <li className="group relative grid grid-cols-[80px_minmax(0,1fr)_auto] items-center gap-3 border-b border-border py-5 transition-[padding,border-color] duration-300 hover:border-accent hover:pl-4 [&:has(a:focus-visible)]:border-accent [&:has(a:focus-visible)]:pl-4 [&:has(a:focus-visible)_img]:scale-[1.06] [&:has(a:focus-visible)_img]:opacity-100 sm:grid-cols-[148px_minmax(0,1fr)_auto] sm:gap-6">
      ...
      <div className="min-w-0 [overflow-wrap:anywhere]">
        <Title className="text-xl font-semibold tracking-tight transition-colors duration-300">
          <Link
            href={`/work/${item.slug}`}
